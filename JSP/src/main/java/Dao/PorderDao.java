...
public class PorderDao extends DB {
	...
	public boolean orderwrite(Porder porder, ArrayList<Cart> carts) {
		...
		try {
			...
			if(rs.next()) {
				...
				for(Cart cart : carts) { // 카트 내 제품 수 만큼 DB에 저장
					...
					// 3. 재고 업대이트 처리 [주문시 주문수량 만큼 제품 재고 차감]
	...
	// 2. 주문 상세 등록
	public ArrayList<Porder> getPorderslist(int m_num) {
	...
	// 상세 목록
	public ArrayList<Porderdetail> getPordersdetail(int order_num) {
