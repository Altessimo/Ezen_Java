...
<body>
	...
	if(id.equals("")||password.equals("")||passwordconfirm.equals("")||name.equals("")||birth.equals("")
		...
		out.print("<script>alert('아이디는 5~15사이로 입력해주세요'); </script>");
		out.println("<script>location.href='../view/member/signup.jsp'; </script>");
	}
	else if(password.length() < 5) {
		out.print("<script>alert('비밀번호는 5~15사이로 입력해주세요'); </script>");
		out.println("<script>location.href='../view/member/signup.jsp'; </script>");
	}
		...
		out.print("<script>alert('연락처는 -를 제외한 11자리로 입력해주세요'); </script>");
		out.println("<script>location.href='../view/member/signup.jsp'; </script>");
	}
	...
	// DB처리
	boolean result = MemberDao.getmemberDao().membersingup(member);
		if(result) { // 회원가입 성공
