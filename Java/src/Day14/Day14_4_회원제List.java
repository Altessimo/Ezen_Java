 ...
 */
	...
	public static void main(String[] args) {
		...
		// 0-2. 설정[리스트 선언 : User 클래스만 담을 수 있는 클래스(User클래스의 하위도 가능)]
		ArrayList<User> users = new ArrayList<>();
		
		// 0-3. 설정[파일 읽어오기 : 파일에 있는 회원들을 리스트에 저장]
		 	// 1. 파일 내 모든 글자 가져오기
		// 1. 파일 입력 클래스
		try {
		FileInputStream fileInputStream = new FileInputStream("C:/Users/505/git/Ezen_Java/Java/src/Day14.txt");
		
		// 1. 파일 내 모든 글자 가져오기[스트림 단위 : 바이트]
			// 1. 바이트 배열 선언
		byte[] bytes = new byte[1024]; // 1bit → 8bit → 1byte → 1024byte → 1kbyte
			// 영문, 공백, 특수문자 : 1 바이트 / 한글 2바이트
		// 2. 읽어오기
		...
		while(true) {
			...
			if(ch==1) {
				...
				// 아이디 중복 체크[리스트 내 객체 하나씩 temp에 대입]
				...
				// 3. 리스트 저장[만약에 중복값이 없으면]
				if(idcheck) {users.add(user); System.out.println("[회원가입 성공]");}
				
				// 4. 파일 처리
					// 1. 파일 출력 클래스(new FileOutputStream("파일경로/파일명.확장자") : 세로쓰기
								// new FileOutputStream("파일경로/파일명.확장자", true); 이어쓰기
				try {
				FileOutputStream faFileOutputStream =
						new FileOutputStream("C:/Users/505/git/Ezen_Java/Java/src/Day14.txt", true);
				
				// 2. 필드 간 구분[,] // 회원 간 구분[\n]
				String outstring = user.getId() + "," + user.getPassword() + "," + user.getName() + "\n";
				
				// 3. String → bytes 내보내기[write(바이트열)]
				faFileOutputStream.write(outstring.getBytes());
			...
			///////////////////////////////// 로그인 성공 시 //////////////////////////////////
						...
						if(ch2==1) {
							...
							try {
								// 0. 파일출력 객체 생성 (파일경로)
							FileOutputStream faFileOutputStream =
									new FileOutputStream("C:/Users/505/git/Ezen_Java/Java/src/Day14.txt");
