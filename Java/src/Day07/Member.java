...
// 3. 메소드[동작]
	// - 회원가입 메소드[입력받은 정보로 객체 생성해서 배열에 저장]
	public static boolean signup() {
		// public : main이 속한 클래스(외부 클래스)로 호출하기 위해
		
		System.out.println("----- 회원가입 페이지 -----");
		System.out.println("아이디 : "); String m_id = Library.scanner.next();
		// 아이디 중복 체크
		...
		System.out.println("비밀번호 : "); String m_pw = Library.scanner.next();
		System.out.println("성명 : "); String m_name = Library.scanner.next();
		System.out.println("연락처 : "); String m_phone = Library.scanner.next();
	...
	// - 로그인 메소드[입력받은 아이디와 비밀번호가 배열 내 존재하면 로그인 성공 아니면 실패]
	public static String login() {
		...
		for(int i=0; i<Library.members.length; i++) {
			if(Library.members[i] != null && Library.members[i].getM_id().equals(m_id) &&
				...
				return m_id; // 로그인 성공시 성공한 아이디 변환
			}
		}
	...
	// - 아이디 찾기 메소드(이름, 연락처 입력 받아 아이디 알려주기)
	public static String findid() {
		...
		for(int i=0; i<Library.members.length; i++) {
			if(Library.members[i] != null && Library.members[i].getM_name().equals(m_name) &&
				...
				return Library.members[i].getM_id(); // 로그인 성공시 성공한 아이디 변환
			}
		}
		
		return null; // 반환값 : 찾았을경우 → 아이디 / 실패시 null
	}
	// - 비밀번호 찾기 메소드(아이디, 연락처 입력받아 비밀번호 알려주기)
