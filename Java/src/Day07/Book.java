...
public class Book {
	// 1. 필드 [ ISBN , 도서명 , 대여여부 , 대여한 회원아이디 등 ]
		...
		public boolean book_add() {
			...
			System.out.println(" 도서코드 : ");	String b_ISBN = Library.scanner.next();
				// 도서코드 중복체크 
		...
		public boolean book_rental(String loginid) {
			...
			System.out.println(" 대여할 도서코드 : ");	String b_ISBN = Library.scanner.next();
				// ISBN 존재여부 체크 
			boolean check = bookcheck(b_ISBN);
			if( check ) {
				...
				for( int i = 0 ; i<Library.books.length ;i++ ) {
					if( Library.books[i] != null && 
						...
						if( Library.books[i].getB_rental() ) {
							...
							Library.books[i].setM_id(loginid);
							return true;
						}
		...
		public boolean book_return(String loginid) {
			...
			System.out.println("반납 도서코드 : ");	String b_ISBN = Library.scanner.next();
			...
			for(int i=0; i<Library.books.length; i++) {
				if(Library.books[i] !=null && ! Library.books[i].getB_rental()) {
					...
					// 3. 로그인된 회원과 = 대여회원
					if(Library.books[i].getM_id().equals(loginid)) {
						System.out.println("[알림] : 도서반납 성공!");
						Library.books[i].setM_id(null); // 1. 반납성공시 대여 회원 → null
						Library.books[i].setB_rental(true); // 2. 대여여부 → true
						return true;
					}
				}
			}
