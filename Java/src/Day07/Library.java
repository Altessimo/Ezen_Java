...
public class Library { // 실행 클래스
	
	/* --------- 회원 도서관 프로그램 ---------
	 [조건 1] : 객체지향 [회원클래스, 도서 클래스]
		- 설계 : 필드, 생성자, 메소드
	 [조건 2] : 객체는 배열에 저장
	 [조건 3] : 회원만 프로그램 이용
		- 로그인전 : 로그인, 회원가입, 아이디/비밀번호 찾기
		- 로그인후 : 도서목록, 대여, 반납, 로그아웃
	 [조건 4] : 관리자(Id : admin Pw : 1234)만 도서 등록 가능
		- 관리자 로그인 : 도서등록, 도서목록
	 */
		// 0. 배열 
	...
	public static void main_menu() {
		while(true) {
			...
			if( ch == 1 ) {
				...
				// 로그인 메소드의 반환 값에 따른 제어
				//1. 로그인 실패
				if(loginid == null) {System.out.println("[알림] 동일한 회원 정보가 없습니다"); }
				// 2. 관리자 메뉴
			...
			if( ch == 3 ) {
				...
				if(findid == null) {System.out.println("[알림] : 동일한 회원 정보가 없습니다"); }
	...
	public static void admin_menu() {
		...
		System.out.println("1. 도서목록 | 2. 대여 | 3. 로그아웃");
		System.out.println("선택 : "); int ch = scanner.nextInt();
