...
public class Day15_QList {
/*
 * 문제1 : List 컬렉션
 * 대기명단 프로그램
 * [메뉴]
 * 1. 명단 추가(인원수를 입력 받아 대기번호[인덱스] 반환)
 * 2. 명단 취소(대기번호 입력 받아 명단 취소)
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(ch==1) {
				...
				System.out.println("취소할 대기번호 : "); int num = scanner.nextInt();
				
				// 2. 리스트에 객체 삭제
				list.remove(num-1); // -1 하는 이유 : 대기번호는 1부터, 인덱스는 0부터
				System.out.println("- 취소되었습니다 -");
			}
			else if(ch==3) {
				if(list.isEmpty()) {
					System.out.println("- 대기중인 고객이 있습니다 -");
				} else {
