...
// 배열을 이용한 정수관리 프로그램
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(ch == 2) {
				// 학생 수 만큼 점수 입력 받기[배열명.length : 배열의길이]
			...
			if(ch == 3) {
				...
				// 클래스 이용한 정렬[해당관련 클래스 = Arrays
				Arrays.sort(학생점수목록); // 오름차순 / 오름차순의 반대는 내림차순
				Arrays.sort(학생점수목록, 	Collections.reverseOrder());
				// Collectors.reverseOrder() : 순서 뒤집기[기본 자료형은 불가, 객체만 가능]
